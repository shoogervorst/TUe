 ...
 */
public class MIPRunner implements Runnable {
    private int t;
    ...
    public MIPRunner(int t, RaycastRenderer renderer, double max, double[] viewVec, double[] vVec, double[] uVec) {
        this.t = t;
    ...
    public void run() {
        ...
        // specified by the UI.
        int steps = Math.max(1,(renderer.panel.mip_raycasting)?(int)max-1:(renderer.interactiveMode)?Math.min(20, renderer.panel.mip_steps):renderer.panel.mip_steps);
        ...
        stepVector[0] =  normalVec[0] * (renderer.panel.mip_raycasting?1:(maxX / (steps + 1)));
        stepVector[1] =  normalVec[1] * (renderer.panel.mip_raycasting?1:(maxY / (steps + 1)));
        stepVector[2] =  normalVec[2] * (renderer.panel.mip_raycasting?1:(maxZ / (steps + 1)));
        ...
        for (int j = 0; j < renderer.image.getHeight(); j+=stepsize) {
            for (int i = t * renderer.image.getWidth() / renderer.NUM_THREADS; i < (t + 1) * renderer.image.getWidth()/renderer.NUM_THREADS; i+=stepsize) {
                        ...
                        )
                {
                    ...
                    // When dragging, do nearest neighbor interpolation
                    val = (renderer.interactiveMode)?renderer.getVoxel(nextCoord):renderer.getVoxel_trilinear(nextCoord);
                ...
                finalColor = renderer.tFunc.getColor(max_val);
